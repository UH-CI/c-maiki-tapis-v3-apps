...
process Lulu {
    ...
    publishDir params.outdir+"Misc/8-LULU_correction", mode: "copy", pattern: "{all_lulu_*.csv}"

    input:
	set val(idThreshold),file(matchlist),file(table) from MATCH_LISTS.join(ABUNDANCE_TABLES)
    output:
        set val(idThreshold),file("all_lulu_${idThreshold}.csv") into ABUNDANCE_LULU
        file("all_lulu_${idThreshold}.csv") into ABUNDANCE_LULU_TO_COUNT
