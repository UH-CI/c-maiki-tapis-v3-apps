...
process Clustering {
    ...
    file("raw*.{fasta,shared,taxonomy,database,biom,relabund}")
...
process Database {
    ...
    set file("*.relabund"), file("*.taxonomy"), file("*.biom"), file("*.database"), file("all_esv.fasta") optional true
...
(FOR_FASTTREE, FOR_CLEARCUT) = [FOR_POSTPROC, Channel.empty()]
...
process FastTree {
    ...
    label "high_computation"
    // label "mothur_script"
    publishDir params.outdir+"Results/postprocessing/unifrac", mode: "copy", pattern: "*.nwk"

    input:
    set val(idThreshold), file(fasta), file(shared) from FOR_FASTTREE

    output:
    set val(idThreshold), file(shared), file("*.nwk") into CLEARCUT_TREE

    script:
    """
    sed -r 's/.*(Otu[0-9]+)\\|.*/\\>\\1/' ${fasta} > relabeled_${fasta}
    FastTree relabeled_${fasta} > FastTree_${idThreshold}.nwk
    """
}
...
process UnifracDistMothur {
    ...
    label "high_computation"
    label "r_script"
    publishDir params.outdir+"Results/postprocessing/unifrac", mode: "copy", pattern: "*{.nwk,summary}"
